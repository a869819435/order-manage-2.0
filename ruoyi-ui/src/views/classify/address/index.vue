<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="地址分类编码" prop="code">
        <el-input
          v-model="queryParams.code"
          placeholder="请输入地址分类编码"
          clearable
          size="small"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['classify:address:add']"
        >新增</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['classify:address:edit']"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['classify:address:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="地址分类编码" prop="code">
          <el-input v-model="form.code" placeholder="请输入地址分类编码" />
        </el-form-item>
        <el-form-item label="地址分类名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入地址分类名称" />
        </el-form-item>
        <el-form-item label="父级id" prop="parentId">
          <treeselect v-model="form.parentId" :options="addressOptions" :normalizer="normalizer" placeholder="请选择父级id" />
        </el-form-item>
        <el-form-item label="层级" prop="level">
...
<script>
import { listAddress, getAddress, delAddress, addAddress, updateAddress, exportAddress } from "@/api/classify/address";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        code: [
          { required: true, message: "地址分类编码不能为空", trigger: "blur" }
        ],
        name: [
  ...
  methods: {
    ...
    getList() {
      ...
      listAddress(this.queryParams).then(response => {
        this.addressList = this.handleTree(response.data, "id", "parentId");
        this.loading = false;
      });
    },
    /** 转换地址分类数据结构 */
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$confirm('是否确认删除地址分类编号为"' + row.id + '"的数据项?', "警告", {
